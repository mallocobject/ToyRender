    ...
    *length = 0;
    ...
    if (*(p + 1) == '#' && *(p + 2)) {
        ...
        while (*q != terminator) {
            ...
            // Reject before adding: if digitScaled alone exceeds
            // MAX_CODE_POINT, or if adding it to ucs would exceed it (checked
            // without overflow by testing ucs > MAX_CODE_POINT - digitScaled,
            // safe since digitScaled
            // <= MAX_CODE_POINT at this point).
            if (digitScaled > MAX_CODE_POINT ||
                ucs > MAX_CODE_POINT - digitScaled) {
                return 0;
            }
            ucs += digitScaled;
            mult *= radix;
...
void XMLUtil::ToStr(uint64_t v, char *buffer, int bufferSize) {
    ...
    TIXML_SNPRINTF(
        buffer, bufferSize, "%llu", static_cast<unsigned long long>(v));
}
...
void XMLPrinter::Write(const char *data, size_t size) {
    if (_fp) {
        ...
        char *p =
            _buffer.PushArr(size) - 1; // back up over the null terminator.
        memcpy(p, data, size);
        p[size] = 0;
    }
}
